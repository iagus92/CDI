 ...
 */
public class LibraryNotify_old {
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            for (int i = 0; i < threads.length; ++i) {
                threads[i].start();
                while(!threads[i].getState().equals(Thread.State.WAITING));
            }
    ...
    static class Reader2 implements Runnable{
        ...
        // Primero pone a esperar a todos los hilos.
                ...
                try {
                    ...
                    while (book.stillPages()) {
                        ...
                        synchronized (readers[next]) {
                            while(!threads[next].getState().equals(Thread.State.WAITING)); // Sobra.
                            readers[next].notify();
                        }
                        readers[hilo].wait();
